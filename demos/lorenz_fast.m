...
c = [0 0 27]; r = [30 30 40]; B = Tree(c,r);    % the box collection
...
n0 = 0; n1 = B.count(depth); tic
while n1 > n0                                   % while boxes are being added
  B.change_flags('all', ins, expd);             % flag inserted boxes for expansion
  b = B.boxes(depth);                           
  flags = b(2*d+1,:); 
  ...
  B.insert(f(P)', depth, ins, none);            % insert boxes which are hit by f(P)
  B.unset_flags('all', expd);                   % unflag recently expanded boxes
  n0 = n1; n1 = B.count(depth); 
  disp(sprintf('%d boxes', n1));
end
toc

%% plot
boxplot3(B); view(20,30); axis square; axis tight;
